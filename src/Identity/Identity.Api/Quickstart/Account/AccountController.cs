...
public class AccountController : Controller
{
    ...
    public AccountController(IIdentityServerInteractionService interaction, IClientStore clientStore, IAuthenticationSchemeProvider schemeProvider, IEventService events, SignInManager<ApplicationUser> signInManager)
    {
    ...
    public async Task<IActionResult> Logout(LogoutInputModel model)
    {
        ...
        if (vm.TriggerExternalSignout)
        {
            ...
            string url = Url.Action("Logout", new { logoutId = vm.LogoutId }) ?? throw new NullReferenceException(nameof(url));
    ...
    public async Task<IActionResult> Register(RegisterViewModel registerViewModel)
    {
        if (ModelState.IsValid)
        {
            ...
            var result = await _signInManager.UserManager.CreateAsync(user, registerViewModel.Password);

            if (result.Succeeded)
            {
                //await _signInManager.UserManager.AddToRoleAsync(user, Constants.Roles.ADMINISTRATORS);
    ...
    {
        var context = await _interaction.GetAuthorizationContextAsync(returnUrl) ?? throw new NullReferenceException(nameof(_interaction.GetAuthorizationContextAsync));
        if (context.IdP is not null && await _schemeProvider.GetSchemeAsync(context.IdP) is not null)
        {
            var local = context.IdP is IdentityServerConstants.LocalIdentityProvider;
